            ...
            <b-col>
                <b-card class="shadow">
                    <b-row class="d-flex justify-content-between align-items-center pb-2">
                        <b-col class="pb-2">
                            <b-button class="c-btn-primary" size="sm" @click="newExpenseCategory"><i
                                    class="fa fa-fw fa-plus-square"></i> New Expense Category
                            </b-button>
                        </b-col>
                    ...
                    <expense-category-modal @saved="getAll" :modal_object="modal_object"></expense-category-modal>
                    <b-row>
                        <b-col>
                                ...
                                <template v-slot:cell(actions)="row">
                                    <div v-if="row.item.role_id != user_role.administrator">
                                        <i class="fa fa-fw fa-edit c-text-success text-success"
                                           @click="updateExpenseCategory(row)"></i>

                                        <i  class="fa fa-fw fa-trash c-text-danger text-danger"
                                            title="Delete Role"
                                            @click="deleteExpenseCategory(row)"></i>
                                    </div>
                                </template>
                            </b-table>
                        </b-col>
...
<script>
    ...
    export default {
        ...
        methods:{
            getAll(){
                this.isBusy = true;
                this.axios.get('api/expense_category').then(
                    response => {
            ...
            updateExpenseCategory(row){
                this.modal_object.status = true;
                this.modal_object.data = row.item;
            },
            deleteExpenseCategory(row){
                this.swalConfirmDelete((data) => {
                    if(data.value){
                        this.axios.delete('api/expense_category/delete', { data: {id:row.item.id} }).then(
                            response => {
                                this.isBusy = false;
                                this.getAll();
                            ...
                            this.swal_transaction_warning_data.text = "Unable to delete, This Exepense Category was already used!";
